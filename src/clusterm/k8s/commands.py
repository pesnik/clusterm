...
class CommandExecutor:
    ...
    def __init__(self, base_path: Path, event_bus: EventBus, logger: Logger):
        ...
        self.current_kubeconfig: Path | None = None
    ...
    def set_kubeconfig(self, kubeconfig_path: Path | None):
    ...
    def _find_helm_binary(self) -> str:
        ...
        # Fall back to system PATH
        try:
            result = subprocess.run(["which", "helm"], check=False, capture_output=True, text=True)
            if result.returncode == 0:
                return result.stdout.strip()
        except Exception:
            pass
    ...
    def _execute_command(self, cmd: list[str], cmd_type: str, timeout: int) -> tuple[bool, str]:
        ...
        try:
            ...
            error_msg = f"Failed to execute {cmd_type} command: {e!s}"
            self.logger.error(error_msg)
            return False, error_msg
